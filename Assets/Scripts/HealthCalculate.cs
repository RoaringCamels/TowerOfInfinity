...
public class Program
{
    ...
    {
        Console.WriteLine("Expression Tree Calculator (Fraction Output)");
        Console.WriteLine("-----------------------------------------");

        // Example with various expressions including negatives
        string[] expressions = {
            "1/3*2",        // Simple multiplication
            "-1/3*2",       // Negative in first term
            "1/3*-2",       // Negative in second term
            "5+-3",         // Handling negative with plus
            "5-3",          // Simple subtraction
            "-5-3",         // Negative first term
            "5*-3",         // Negative multiplication
            "(-5)*3",       // Negative in parentheses
            "1/(-3)*2"      // Negative in denominator
        };
        ...
        foreach (string expression in expressions)
        {
            try
            {
                ...
                Console.WriteLine("Expression tree (inorder):");
        ...
        while (true)
        {
            ...
            if (!string.IsNullOrEmpty(input))
            {
                try
                {
                    ...
                    newTree.BuildFromInfix(input);
                    Console.WriteLine("Expression tree (inorder):");
                    newTree.InorderTraversal();
                    Console.WriteLine($"Result: {newTree.Evaluate()}");
                }
                catch (Exception ex)
                {
